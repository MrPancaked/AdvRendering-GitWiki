...
namespace core {
    ...
    void ParticleManager::UpdateParticles(float& deltaTime) {
        ...
        for (int i = 0; i < particleAmount; i++) {
            ...
            velocities[i] += gravityComp; //+ pressureComp;
    ...
    glm::vec2 ParticleManager::CalculatePressureGradient(const glm::vec2& location) const {
        ...
        for (int i = 0; i < particleAmount; i++) {
            ...
            float slope = 4.0f * glm::pi<float>() * glm::pow(smoothingRadius, 3.0f) * distance / 3.0f - 4.0f * glm::pi<float>() * glm::pow(smoothingRadius, 4.0f) / 3.0f;
